...
def _dead_example(key, base, res):  return f"V + J   {key}   {base}   →   {res}"
def _dir_produces(char, name):      return f"{char}  {name}" if name else char
def _dir_example(key, char):        return f"V + J   {key}   →   {char}"
...
if "--md" in sys.argv:
    ...
    for key, mark, name, base, res in DEAD:
        ...
        md.append(f"| `{key}` | {m} {name} (dead key) | `V + J`, `{key}`, `{base}` → {res} |")
    for key, char, name in DIRECT:
        prod = f"{char} ({name})" if name else char
        md.append(f"| `{key}` | {prod} | `V + J`, `{key}` → {char} |")
    footnote = ("Armed from the **base layer** with V + J. Dead keys wait for a base "
...
add(f'<text x="{MX}" y="78" font-size="13.5" fill="{MUTED}">'
    'Press V + J on the base layer to arm Compose, then press one key.</text>')
...
add(f'<text x="{MX+18}" y="{CARD_TOP+50}" font-size="14.5" fill="{INK}">'
    '<tspan font-weight="600">V + J</tspan>'
